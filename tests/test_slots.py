...
@pytest.mark.skipif(not PY_3_8_PLUS, reason="cached_property is 3.8+")
def test_slots_cached_property_conflicting_inherited_field_raises():
    """
    A cached_property that collides with an inherited attrs field raises a
    deterministic error instead of being silently dropped.
    """
    ...
    with pytest.raises(ValueError, match=r"conflict with attrs fields.*f"):
...
@pytest.mark.skipif(not PY_3_8_PLUS, reason="cached_property is 3.8+")
def test_slots_cached_property_conflicting_these_field_raises():
    """
    A cached_property that collides with a field defined via *these* raises
    a deterministic error instead of being silently dropped.
    """
    with pytest.raises(ValueError, match=r"conflict with attrs fields.*f"):

        @attr.s(slots=True, these={"f": attr.ib()})
        class C:
            @functools.cached_property
            def f(self):
                return 1
...
@pytest.mark.skipif(not PY_3_8_PLUS, reason="cached_property is 3.8+")
def test_slots_cached_property_conflicting_explicit_slot_raises():
    """
    A cached_property that collides with an explicit __slots__ entry of the
    same class raises a deterministic error instead of silently replacing
    the requested slot.
    """
    ...
    C.f = functools.cached_property(lambda self: 1)

    with pytest.raises(ValueError, match=r"explicit __slots__ entries.*f"):
        attr.s(slots=True)(C)


@pytest.mark.skipif(not PY_3_8_PLUS, reason="cached_property is 3.8+")
def test_slots_cached_property_weakref_slot():
    """
    weakref_slot keeps working on classes with cached properties.
    """
...
@pytest.mark.skipif(not PY_3_8_PLUS, reason="cached_property is 3.8+")
def test_slots_cached_property_frozen_caches_but_stays_frozen():
    """
    On frozen classes the cached value is written to the slot despite the
    class being frozen, while regular attribute writes stay blocked.
    """
...
if PY_3_8_PLUS:
    # Module-level so instances are picklable with the stdlib pickle.
    # Guarded because functools.cached_property is 3.8+.
    @attr.s(slots=True)
    class AWithCachedProperty:
        x = attr.ib()
...
@pytest.mark.skipif(not PY_3_8_PLUS, reason="cached_property is 3.8+")
def test_slots_cached_property_pickle_roundtrip():
    """
    Instances of slotted classes with cached properties can be pickled; the
    cache itself is not pickled and is recomputed on access.
    """
    a = AWithCachedProperty(1)
    assert a.f == 1
