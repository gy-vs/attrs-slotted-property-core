...
def simple_classes(
    ...
    By default, some combinations of classes will contain a cached_property
    attribute or not. If `cached_property=True` is passed in, all classes
    will contain a cached_property, and if `cached_property=False` is passed
    in, no classes will contain a cached_property.
    """
    ...
    bases = (object,)
    if PY_3_8_PLUS and (
        cached_property or (cached_property is None and cached_property_flag)
    ):
